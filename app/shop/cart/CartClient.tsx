  ...
  return (
    <div className="pt-24 bg-[#F5F4F0] min-h-screen">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        {!ready ? (
          <div className="h-40 rounded-2xl bg-white border border-[#E8E6E0] animate-pulse" />
        ) : entries.length === 0 ? (
          <div className="bg-white rounded-2xl border border-[#E8E6E0] p-10 text-center">
            ...
            <Link
              href="/shop"
              className="inline-flex items-center gap-2 bg-[#FF6B00] text-white font-semibold px-7 py-3 rounded-full
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 items-start">
            {/* ── Позиции ── */}
            <ul className="bg-white rounded-2xl border border-[#E8E6E0] divide-y divide-[#E8E6E0]">
              {entries.map(({ item, qty, sum }) => (
                <li key={item.id} className="flex gap-4 p-4">
                  <Link
                    href={`/shop/${item.slug}`}
                    className="relative w-20 h-20 sm:w-24 sm:h-24 shrink-0 rounded-xl overflow-hidden bg-white border border-[#E8E6E0]

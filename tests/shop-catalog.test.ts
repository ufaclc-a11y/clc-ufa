...
/**
 * Товары без габаритов. Список пуст: с переходом выгрузки на Content API
 * продавца габариты и вес есть у всех позиций. Публичная карточка WB отдавала
 * их не для всех категорий, из-за чего 14 товаров считались «незаполненными».
 *
 * Если товар всё же появится здесь — расчёт доставки по нему работать не будет,
 * покупатель увидит «стоимость сообщит менеджер».
 */
const KNOWN_INCOMPLETE_PACKAGING = new Set<number>([])
